...
| Unified Memory | HAOS VM | LLM Budget | What You Get |
|----------------|---------|------------|--------------|
| **16GB** | 4GB | ~8-9GB | Small model (7-8B params). Basic conversation and device control. Explicit tool calling only — the model does what you ask but won't proactively store facts or make smart decisions. |
| **24GB** | 4GB | ~16-17GB | Mid-size model (14-15B params). Noticeably better conversation. Tool calling possible but reliability varies by model — test before relying on it. |
| **32GB** | 4GB | ~24-25GB | Large model (30B+ params). Reliable proactive tool calling — the model detects personal facts and stores them without being asked. Full voice pipeline with persistent memory. This is what we developed and tested on. |

**Tested environment:** Mac Mini M4, 32GB unified memory, 1TB SSD, macOS Sequoia.

> **Note:** The LLM budget above accounts for macOS (~3-4GB), the HAOS VM (4GB allocated), and the embedding model (~0.5GB if using persistent memory). Your actual headroom may vary.
...
The base M4 Mac Mini (16GB, $599) gets you started. The 32GB model is recommended if you want reliable tool calling and persistent memory.
...
3. Select **Emulate** (not Virtualize — HAOS needs QEMU for its bootloader)
4. Choose **Other**
...
The default HAOS disk is only 6GB. Expand it before first boot:
...
For HAOS to be accessible on your LAN (required for Ollama connectivity later):
...
This gives the VM its own IP address on your LAN.

### 1.7 First Boot

1. Start the VM in UTM
2. Wait 2-5 minutes for HAOS to initialize on first boot
...
**Verify:**
```bash
# From your Mac terminal, find the VM's IP and test connectivity
# Replace with your VM's actual IP (check your router or the VM console)
...
By default, Ollama only listens on localhost. The HAOS VM needs to reach it, so we need to listen on all interfaces.
...
Add these environment variables to the plist (in the `EnvironmentVariables` dict):

| Variable | Value | Why |
|----------|-------|-----|
| `OLLAMA_HOST` | `0.0.0.0` | Listen on all interfaces so the VM can reach Ollama |
...
> **Warning:** As of early 2026, Piper v2.2.1 has a known bug (`espeakbridge` ImportError). If you hit this, check the [GitHub issue](https://github.com/home-assistant/addons/issues/4384) for the current status. The workaround is to pin to v2.1.1.
...
The speaker should appear in Music Assistant as a player. For auto-start on boot, create a launchd plist.
...
3. Install, then go to **Configuration**:
   - Disable **Protection Mode** (allows full filesystem access)
   - Add your SSH public key (generate one with `ssh-keygen` if needed)
4. Start the addon
...
Key principles:
- **"CRITICAL RULE" language at the top** — overrides the HA Assist API preamble which tells the model to "answer from internal knowledge"
...
2. Check it's listening on all interfaces: `curl http://YOUR_MAC_IP:11434/api/tags` (from your Mac — use the LAN IP, not localhost)
3. If only localhost works, `OLLAMA_HOST=0.0.0.0` isn't set — check your Ollama launchd plist
